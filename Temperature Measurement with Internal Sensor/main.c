...
//	GPIO_CLK_EN('F');
 ...
 // GPIO_Init('F',1,1,1,1,0,0,0);    
 // GPIO_Init('F',2,1,1,1,0,0,0); 
	GPIO_Init('C',4,0,1,1,0,0,0);
...
void ADC0_Init(void){
	...
	SYSCTL_RCGC0_R &=~0x00000300; // Determine sample speed to be 125 ksample / sec
	ADC0_ACTSS_R &= ~0x8; // Active Sample Sequencer Register disable ss3 during config.
	ADC0_EMUX_R &= ~0xF000; // Event Multiplexer Register Processor Trigger Sequencer to start Sampling
	ADC0_SSCTL3_R |= 0xe;   // Sample Sequencer 3 Control Register one sample at atime(SS3), set flag(When conversion ends it is cleared), temp sensor
